...
`profiles`, `leagues`, `memberships` (role + `is_commissioner` + status), `sleeper_league_users`, `events` (all instants UTC, `timezone` = Africa/Johannesburg), `itinerary_items`, `challenges`, `penalties`, `press_prompts`, `evidence_submissions` (versioned: draft → submitted → approved | flagged | superseded), `evidence_files`, `review_decisions` (audit, unique idempotency key), `location_settings`, `checkins`, `member_locations` (one upserted pin per league member), `activity_posts`, `reactions` (one per member per post), `props` (over/under and yes/no, per-prop lock, result), `prop_picks` (one side per member per prop), `prediction_rules`, `predictions`, `official_results`, `prediction_awards`, `certificates`.
...
- `share_member_location` / `clear_member_location` / `event_member_locations` — any active league member places, moves or removes one pin (“where everyone is”), labelled from the gazetteer, with no history and no feed post; members read every pin joined with the profile’s name and kit. Only the participant’s check-ins are drawn as a route.
...
- `upsert_prop_pick` — one side per prop, editable until the prop locks; other members’ picks are hidden until then. `settle_prop` (commissioner, locked props only and only once `events.resolution_opened_at` is set, idempotent, posts to the feed; `void` scores nothing). `prop_leaderboard` scores active members: one point per correct pick. `upsert_props` (commissioner) writes the board the app generates from the programme (`src/lib/prop-generator.ts`); refused once locked or picked.
- `upsert_prediction` — rejects writes at/after `events.prediction_lock_at` (the Pofadder arrival); `predictions` RLS (and so `predictions_revealed`) hides others until `events.resolution_opened_at` is set; `resolve_predictions` (commissioner, refused until then) awards closest/exact with shared ties. `set_resolution_open` (commissioner, the "Resolve props and predictions" button on Review) is the only way to set it: nothing opens on a timer; refused before the lock; closing again only while nothing is settled or resolved.
...
- `set_final_checkin` — participant (or commissioner) confirms the last check-in of the trip; the certificate PNG draws the route from the first check-in to that one with the participant's kit badge. Refused once the certificate is issued.
...
1. **Supabase project**: `npx supabase link --project-ref <ref>` then `npm run db:push` (migrations) and run `supabase/seed.sql` (SQL editor or `psql`). In **Auth → Sign In / Providers** turn **off** *Allow new users to sign up* (invite-only) but keep the **Email provider enabled** (disabling the provider blocks all email logins, including invites). In **Auth → URL configuration** set the site URL to your origin and add `<origin>/auth/confirm` to redirect URLs. **Email delivery:** Supabase’s built-in email service is for testing only (a few emails per hour, and on the free tier it only delivers to your own organisation members, so league invites will not arrive). Configure **custom SMTP** (for example Resend or Postmark) under **Project Settings → Auth → SMTP** before inviting the league. Once custom SMTP is on you may also paste `supabase/templates/invite.html` into *Invite user* and `supabase/templates/magic_link.html` into *Magic link* (template editing is blocked on the free tier with the default provider). The app works with the default templates too: `/auth/confirm` accepts `token_hash`, PKCE `?code=` and implicit `#access_token` links. Confirm the `evidence` bucket exists and is private (the storage migration creates it).
2. **Environment** (see `.env.example`): `PB_TESTING_RESET` (server only; `true` on the testing deployment only), `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, `SUPABASE_SECRET_KEY` (server only, needed for invites), `NEXT_PUBLIC_APP_ORIGIN`, `NEXT_PUBLIC_LEAGUE_SLUG`, `NEXT_PUBLIC_EVENT_SLUG`, `SLEEPER_LEAGUE_ID`, `NEXT_PUBLIC_SUPABASE_RESUMABLE_URL` (`https://<ref>.storage.supabase.co/storage/v1/upload/resumable` on hosted projects), `NEXT_PUBLIC_MAP_TILE_URL` + `NEXT_PUBLIC_MAP_TILE_ATTRIBUTION` (tile provider; defaults to public OpenStreetMap tiles, set the URL to `static` for the labelled fallback preview), `MAP_ROUTING_URL` (server only; OSRM router used to draw the check-in line along real roads, defaults to the public demo router, `none` for straight lines).
...
## Testing and league environments

Two deployments share this repository and never share data:
