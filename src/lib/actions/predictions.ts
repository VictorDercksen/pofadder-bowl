...
const schema = z.object({ hours: z.number().int(), minutes: z.number().int(), mealRating: z.number().int(), complaints: z.number().int() });
...
export async function savePrediction(input: z.input<typeof schema>): Promise<ActionResult> {
  ...
  const { error } = await ctx.supabase.rpc("upsert_prediction", {
    ...
    p_complaint_count: parsed.data.complaints,
  });
...
export async function updatePredictionRules(input: { runPoints: number; mealPoints: number; complaintsPoints: number }): Promise<ActionResult> {
