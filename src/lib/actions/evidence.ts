...
export async function deleteDraftFile(input: { fileId: string }): Promise<ActionResult> {
  ...
  const { error } = await ctx.supabase.from("evidence_files").delete().eq("id", file.id);
  if (error) return { ok: false, message: "Only draft files can be removed." };
  await ctx.supabase.storage.from(BUCKET).remove([file.storage_path]);
  revalidateEvidence();
  return { ok: true, message: "File removed from the draft." };
}
...
/** Fresh short-lived signed read URL for a private evidence file (members only via RLS). */
export async function signedMediaUrl(input: { fileId: string }): Promise<{ ok: true; url: string; mime: string; kind: string; name: string } | { ok: false; message: string }> {
  ...
  if (!parsed.success) return { ok: false, message: "Invalid file." };
  const ctx = await getLeagueContext();
  const { data: file } = await ctx.supabase.from("evidence_files").select("storage_path, mime_type, kind, original_name").eq("id", parsed.data.fileId).maybeSingle();
  if (!file) return { ok: false, message: "File not found." };
