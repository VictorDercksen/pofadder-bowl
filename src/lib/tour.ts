...
const commissionerBlock: TourStep[] = [
  ...
  step("review-tools", "THE SCOREBOARD SIDE", "Penalties, results, certificate.", "Flag a penalty, enter the official results (the league's predictions resolve against them) and issue the certificate that closes the event. Nothing settles on a timer: press Resolve props and predictions to reveal the slips and open the settle buttons, then settle the locked props on the prop board and resolve the slips here.", "/review", ["review-tools"]),
];
...
const leagueBlock = (participant: boolean): TourStep[] => [
  ...
  step("map", "SCREEN 03", "Check-in map.", "The route line grows from Malmesbury to Pofadder and back, oldest to newest. The badge in the participant’s kit with the orange halo is the latest check-in; the dots behind it are earlier ones and town pins are references only. The history alongside shows capture time, receive time and accuracy.", "/map", ["map-panel"]),
  ...(participant
    ? []
  ...
  step("props", "SCREEN 06", "The prop board.", "Over/unders on the trip itself. Pick a side on each prop before it locks; the others' picks stay hidden until then. The commissioner settles each prop once resolution opens, and the standings alongside update live.", "/props", ["prop-board"]),
  step("predictions", "SCREEN 07", "Call it before kickoff.", "Seven calls: the 10 km finish time, the rib rating, the final score, when the daylight sign photo lands, how many versions get flagged, the distance on the trace and the speech length. The slip locks when the bus reaches Pofadder and stays hidden from the others until the commissioner opens resolution. Closest call takes the points once the official results are in and the slips are resolved.", "/predictions", ["prediction-slip"]),
  step(
    "press",
    "SCREEN 08",
    "Face the press.",
